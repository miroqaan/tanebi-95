...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE handle, EFI_SYSTEM_TABLE* st)
{
	...
	// Enter MAP01 immediately at a combat-ready difficulty so QEMU launches into real
	// gameplay instead of leaving the user at a passive title/demo screen.
	const CHAR16* argv[] = {
		L"doom.exe", L"-warp", L"1", L"-skill", L"1", NULL
	};
	SerialPortWrite("DOOM: entering libc\n", 20);
	return ShellAppMain(5, argv);
}
